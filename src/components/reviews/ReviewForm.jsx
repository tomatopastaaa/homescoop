    ...
    <div className="flex gap-1 mt-1">
      {[1,2,3,4,5].map(n => (
        <button
          ...
          type="button"
          onClick={() => onChange(n)}
          className={`text-2xl transition-colors ${n <= value ? 'text-amber-400' : 'text-ink-200'} hover:text-amber-400`}
        >★</button>
      ))}
    </div>
  )
...
function SectionHeader({ icon, label }) {
  return (
    <div className="flex items-center gap-2 text-sm font-medium text-ink-700 border-b border-ink-100 pb-2 mb-3 mt-5">
      <span>{icon}</span>
      <span>{label}</span>
    </div>
  )
...
const EMPTY_FORM = {
  firm_name: '', designer_name: '',
  overall_rating: 0, verdict: 'mixed',
  p_quote: '', p_expect: '', p_comms: '', p_specs: '', p_notes: '',
  e_visits: '', e_pm: '', e_issues: '', e_vo: '', e_cost: '',
  e_work: '', e_defects: '', e_vs: '', e_again: '', e_notes: '',
  c_name: '', c_source: '', c_rating: 0, c_exp: '', c_notes: '',
  source_url: '',
}
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data: firmData } = await supabase
        .from('firms')
        .select('id')
        .ilike('name', form.firm_name.trim())
        .maybeSingle()
      ...
      if (firmData) {
        ...
        const { data: newFirm } = await supabase
          .from('firms')
          .insert({ name: form.firm_name.trim(), market_id: 'sg', verdict, tags })
          .select('id')
          .single()
        firm_id = newFirm?.id
      }
      ...
      if (form.designer_name.trim()) {
        const { data: designerData } = await supabase
          .from('designers')
          .select('id')
          .ilike('name', form.designer_name.trim())
          .maybeSingle()
        ...
        if (designerData) {
          ...
          const { data: newDes } = await supabase
            .from('designers')
            .insert({ name: form.designer_name.trim(), market_id: 'sg', verdict, tags })
            .select('id')
            .single()
    ...
    return (
      <div className="fixed inset-0 bg-black/40 z-50 flex items-start justify-center pt-20 px-4">
        <div className="bg-white rounded-2xl p-8 w-full max-w-sm text-center shadow-xl">
          ...
          <button onClick={onClose} className="mt-3 text-sm text-ink-400 hover:text-ink-600">
            {t('form.cancel')}
          </button>
        </div>
      </div>
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-start justify-center pt-6 px-4 pb-6">
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[92vh] overflow-y-auto shadow-xl">
        <div className="sticky top-0 bg-white border-b border-ink-100 px-6 py-4 rounded-t-2xl flex items-center justify-between z-10">
          <div>
            <h2 className="font-display text-xl text-ink-900">{t('form.title')}</h2>
            <p className="text-xs text-ink-500 mt-0.5">{t('form.sub')}</p>
          </div>
        ...
        <div className="px-6 py-4">
          <SectionHeader icon="🏢" label={t('form.basicInfo')} />
          <div className="grid grid-cols-2 gap-3">
            <Field label={`${t('form.firmName')} ${t('form.required')}`}>
              <input
                type="text" value={form.firm_name} onChange={setRaw('firm_name')}
            ...
            <Field label={t('form.designerName')} optional>
              <input
                type="text" value={form.designer_name} onChange={setRaw('designer_name')}
                placeholder={t('form.designerPlaceholder')}
                className="w-full text-sm border border-ink-200 rounded-lg px-3 py-2 focus:outline-none focus:border-ink-400"
              />
            </Field>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Field label={`${t('form.overallRating')} ${t('form.required')}`}>
              <Stars value={form.overall_rating} onChange={set('overall_rating')} />
            </Field>
            <Field label={t('form.verdict')}>
              <Select id="verdict" value={form.verdict} onChange={set('verdict')}
                options={['good','mixed','bad']}
              />
            </Field>
          </div>

          <SectionHeader icon="📋" label={t('review.planning')} />
          <div className="grid grid-cols-2 gap-3">
          ...
          <SectionHeader icon="🔨" label={t('review.execution')} />
          <div className="grid grid-cols-2 gap-3">
          ...
          <SectionHeader icon="🛠" label={t('review.contractor')} />
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label={t('form.contractorRating')} optional>
              <Stars value={form.c_rating} onChange={set('c_rating')} />
            </Field>
          ...
          <SectionHeader icon="🔗" label={t('form.source')} />
